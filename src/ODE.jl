...
function Base.convert(::Type{Tnew}, tab::Tableau) where {Tnew <: Real}
...
function make_consistent_types(fn, y0, tspan, btab::Tableau)
    ...
    !isconcretetype(Et) &&
        @warn("The eltype(tspan) is not a concrete type!  Change type of tspan for better performance.")
    !isconcretetype(Eyf) &&
        @warn("The eltype(y0/tspan[1]) is not a concrete type!  Change type of y0 and/or tspan for better performance.")
...
function ode4s_kr(F, x0, tspan; jacobian = nothing, kwargs...)
    oderosenbrock(F, x0, tspan, kr4_coefficients...; jacobian = jacobian, kwargs...)
end
...
function ode4s_s(F, x0, tspan; jacobian = nothing, kwargs...)
...
function ode4s(F, x0, tspan; jacobian = nothing, kwargs...)
