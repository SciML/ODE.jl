...
# type FixedState <: AbstractState
#     t; dt; y; tmp :: TempArrays
# end


immutable FixedProblem <: AbstractProblem
    F
    method
    y0
    t0
...
# # generates the iterator from TableauRKExplicit
# function call(tab::TableauRKExplicit, fn, y0, t0, dt0; tstop = Inf)
#     return FixedProblem(fn, tab, y0, t0, dt0, tstop)
# end

# function start(problem :: FixedProblem)
#     t0  = problem.t0
#     dt0 = problem.dt0
#     y0  = problem.y0
#     tmp = TempArrays(similar(y0), Array(typeof(y0), S(problem.btab)), Void())
#     return FixedState(t0, dt0, y0, tmp)
# end


function next(prob :: FixedProblem, state :: State)
    dof = length(state.y)
...
# function done(prob :: FixedProblem, state :: FixedState)
#     return state.t >= prob.tstop # || abs(state.dt) < prob.minstep
# end
...
immutable AdaptiveProblem <: AbstractProblem
    F
    method
    y0
    t0
...
# function AdaptiveProblem(fn, y0, t0; tstop = Inf, method = bt_feuler, reltol = 1e-5, abstol = 1e-5, minstep = 1e-10, maxstep = 1/minstep)
#     return AdaptiveProblem{:adaptive}(fn, method, y0, t0, tstop, reltol, abstol, minstep, maxstep)
# end
...
function solver(F, y0, t0;
    ...
    if isadaptive(method)
        return AdaptiveProblem(F, method, y0, t0, dt0, tstop, reltol, abstol, minstep, maxstep)
    else
        return FixedProblem(F, method, y0, t0, dt0, tstop)
    end
end


function start(problem :: AbstractProblem)
...
function done(prob :: AbstractProblem, state :: State)
    return state.t >= prob.tstop || state.dt < prob.minstep
end


function next(prob :: AdaptiveProblem, state :: State)
...
function rk_embedded_step!(tmp :: TempArrays, state :: State, prob :: AdaptiveProblem, dt)
...
function calc_next_k!(tmp :: TempArrays, i, state :: State, prob :: AbstractProblem)
    dof = length(state.y)
